...
RAG_DEBUG_LOG_TOP_N=20
```

日志写入 `backend/logs/app.log`，会记录 retrieve、rrf、rerank、filtered、expanded 各阶段的 chunk 排名、文档名、Qdrant 分数、RRF 分数、Rerank 分数和摘要，用于定位无关文档是在召回、融合、重排、过滤还是相邻 chunk 扩展阶段进入上下文。

## 4. 流式进度策略

聊天接口会在耗时节点通过 SSE 推送 `progress` 事件，前端将临时的“正在思考...”替换为当前阶段文案，例如识别问题类型、改写检索问题、检索知识库、筛选相关资料和生成回答。进度事件只用于当前页面反馈，不保存到数据库；首个回答 token 到达后会替换为正式回答内容。
...
- 回答末尾的知识来源应基于进入 Prompt 的 chunk 摘要生成；后端仍保存 `message_sources` 供审计和调试使用。
...
- 知识库内问题：应能回答，并在回答末尾按文档聚合展示命中 chunk 摘要。
